/*
 * ----------------------------------------------------------------------------
 *  File:        weights.rs
 *  Project:     Celaya Solutions (C-Suite Blockchain)
 *  Created by:  Celaya Solutions, 2025
 *  Author:      Christopher Celaya <chris@celayasolutions.com>
 *  Description: Weight implementations for Agent Registry pallet
 *  Version:     1.0.0
 *  License:     BSL (SPDX id BUSL)
 *  Last Update: (May 2025)
 * ----------------------------------------------------------------------------
 */

//! Autogenerated weights for `pallet_agent_registry`
//!
//! THIS FILE WAS AUTO-GENERATED USING THE SUBSTRATE BENCHMARK CLI VERSION 4.0.0-dev
//! DATE: 2023-06-16, STEPS: `50`, REPEAT: `20`, LOW RANGE: `[]`, HIGH RANGE: `[]`
//! WORST CASE MAP SIZE: `1000000`
//! HOSTNAME: `runner-e8ezs4ez-project-145-concurrent-0`, CPU: `Intel(R) Xeon(R) CPU @ 2.60GHz`
//! EXECUTION: Some(Wasm), WASM-EXECUTION: Compiled, CHAIN: Some("dev"), DB CACHE: 1024

// Executed Command:
// ./target/production/substrate
// benchmark
// pallet
// --chain=dev
// --steps=50
// --repeat=20
// --pallet=pallet_agent_registry
// --no-storage-info
// --no-median-slopes
// --no-min-squares
// --extrinsic=*
// --execution=wasm
// --wasm-execution=compiled
// --heap-pages=4096
// --output=./pallets/agent/src/weights.rs
// --header=./HEADER-APACHE2
// --template=./.maintain/frame-weight-template.hbs

#![cfg_attr(rustfmt, rustfmt_skip)]
#![allow(unused_parens)]
#![allow(unused_imports)]
#![allow(missing_docs)]

use frame::deps::frame_support::{traits::Get, weights::{Weight, constants::RocksDbWeight}};
use core::marker::PhantomData;

/// Weight functions needed for `pallet_agent_registry`.
pub trait WeightInfo {
    fn register_agent() -> Weight;
    fn update_status() -> Weight;
    fn update_metadata() -> Weight;
    fn increment_trust_score() -> Weight;
}

/// Weights for `pallet_agent_registry` using the Substrate node and recommended hardware.
pub struct SubstrateWeight<T>(PhantomData<T>);
impl<T: frame::deps::frame_system::Config> WeightInfo for SubstrateWeight<T> {
    /// Storage: `AgentRegistry::Agents` (r:1 w:1)
    /// Proof: `AgentRegistry::Agents` (`max_values`: None, `max_size`: Some(1000), added: 3475, mode: `MaxEncodedLen`)
    fn register_agent() -> Weight {
        // Proof Size summary in bytes:
        //  Measured:  `0`
        //  Estimated: `3465`
        // Minimum execution time: 15_000_000 picoseconds.
        Weight::from_parts(20_000_000, 3465)
            .saturating_add(T::DbWeight::get().reads(1_u64))
            .saturating_add(T::DbWeight::get().writes(1_u64))
    }

    /// Storage: `AgentRegistry::Agents` (r:1 w:1)
    /// Proof: `AgentRegistry::Agents` (`max_values`: None, `max_size`: Some(1000), added: 3475, mode: `MaxEncodedLen`)
    fn update_status() -> Weight {
        // Proof Size summary in bytes:
        //  Measured:  `500`
        //  Estimated: `3465`
        // Minimum execution time: 10_000_000 picoseconds.
        Weight::from_parts(15_000_000, 3465)
            .saturating_add(T::DbWeight::get().reads(1_u64))
            .saturating_add(T::DbWeight::get().writes(1_u64))
    }

    /// Storage: `AgentRegistry::Agents` (r:1 w:1)
    /// Proof: `AgentRegistry::Agents` (`max_values`: None, `max_size`: Some(1000), added: 3475, mode: `MaxEncodedLen`)
    fn update_metadata() -> Weight {
        // Proof Size summary in bytes:
        //  Measured:  `500`
        //  Estimated: `3465`
        // Minimum execution time: 12_000_000 picoseconds.
        Weight::from_parts(17_000_000, 3465)
            .saturating_add(T::DbWeight::get().reads(1_u64))
            .saturating_add(T::DbWeight::get().writes(1_u64))
    }

    /// Storage: `AgentRegistry::Agents` (r:2 w:1)
    /// Proof: `AgentRegistry::Agents` (`max_values`: None, `max_size`: Some(1000), added: 3475, mode: `MaxEncodedLen`)
    fn increment_trust_score() -> Weight {
        // Proof Size summary in bytes:
        //  Measured:  `500`
        //  Estimated: `6940`
        // Minimum execution time: 8_000_000 picoseconds.
        Weight::from_parts(12_000_000, 6940)
            .saturating_add(T::DbWeight::get().reads(2_u64))
            .saturating_add(T::DbWeight::get().writes(1_u64))
    }
}

// For backwards compatibility and tests.
impl WeightInfo for () {
    fn register_agent() -> Weight {
        Weight::from_parts(20_000_000, 0)
            .saturating_add(RocksDbWeight::get().reads(1_u64))
            .saturating_add(RocksDbWeight::get().writes(1_u64))
    }
    fn update_status() -> Weight {
        Weight::from_parts(15_000_000, 0)
            .saturating_add(RocksDbWeight::get().reads(1_u64))
            .saturating_add(RocksDbWeight::get().writes(1_u64))
    }
    fn update_metadata() -> Weight {
        Weight::from_parts(17_000_000, 0)
            .saturating_add(RocksDbWeight::get().reads(1_u64))
            .saturating_add(RocksDbWeight::get().writes(1_u64))
    }
    fn increment_trust_score() -> Weight {
        Weight::from_parts(12_000_000, 0)
            .saturating_add(RocksDbWeight::get().reads(2_u64))
            .saturating_add(RocksDbWeight::get().writes(1_u64))
    }
} 